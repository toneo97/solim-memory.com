...
interface PaymentPageProps {
  ...
  wizardData?: any;
  onStartWizard?: () => void;
  showSuccessView?: boolean; // New prop to force success view rendering
}
...
const PaymentPage: React.FC<PaymentPageProps> = ({ 
  ...
  // Initialize step if showSuccessView is true
  useEffect(() => {
    if (showSuccessView) {
      setStep('success');
    }
  ...
  const handlePaymentSuccess = () => {
    // Redirect to the dedicated success URL
    window.history.pushState(null, '', '/memory-reliv/success');
    // Force a reload or notify App to update view - since App.tsx listens to pushState usually it's manual, 
    // but here we might need to manually trigger the popstate event or let App handle it if passed down.
    // However, since we are inside App which controls routing, simply calling pushState won't trigger re-render in App unless we use the navigate function.
    // To keep it simple without passing navigate prop deep down (though we could), we can just force reload or rely on App re-rendering if we were using a router library.
    // Since App.tsx has a navigate function, ideally we would use that.
    // BUT, we are currently inside PaymentPage.
    // Let's assume on successful payment we just update local state to success IF we are not using the router method strictly,
    // OR we trigger the URL change and let App re-mount us with showSuccessView=true.
    
    // Dispatch a popstate event to notify App.tsx (since we use custom router there)
    window.history.pushState(null, '', '/memory-reliv/success');
  ...
  // ... (Rest of component matches original content for 'selection' and 'checkout' steps) ...
  return (
                  ...
                  <tbody className="divide-y divide-slate-50">
                    {[
                      ...
                      <tr key={idx} className={idx % 2 === 0 ? "bg-white" : "bg-slate-50/30"}>
                        <td className="p-4 pl-6 md:pl-10 text-slate-500 font-medium text-sm">{(row as any).feature}</td>
                        <td className="p-4 text-center text-slate-500 text-sm">{(row as any).clip}</td>
