...
import { ChevronLeft, ChevronRight, Mic, Square, MapPin, Upload, Search, Music, Clock, Cloud, Users, User, CreditCard, ChevronDown } from 'lucide-react';
...
export const initialPerson: PersonDetails = {
...
const IntakeWizard: React.FC<IntakeWizardProps> = ({ 
  ...
  }, [currentStep, personIndex]);

  const updateData = (field: keyof WizardFormData, value: any) => {
  ...
  const nextStep = () => {
    ...
    if (currentStep === 7) {
       ...
       if (formData.userName && !initialPeopleArray[0].name) {
           initialPeopleArray[0].name = formData.userName;
       }
       const mergedPeople = initialPeopleArray.map((p, i) => formData.people[i] || p);
       updateData('people', mergedPeople);
       setPersonIndex(0);
    }
          ...
          <>
            ...
            <div className="grid md:grid-cols-2 gap-6 w-full">
              {['First Person (POV)', 'Third Person'].map((opt) => (
                 <button
                 key={opt}
                 onClick={() => updateData('pov', opt as any)}
  ...
  const getOrbText = () => {
    const s = currentStep;
    if (s === 1) return "Welcome. To help us personalize your experience, please confirm your details.";
    if (s === 2) return "Can you tell us, in your own words, about the memory you’d like to revisit?";
    if (s === 3) return "Can you tell us roughly when this memory took place? What month and year was it?";
    if (s === 4) return "Where exactly did this memory unfold?";
    if (s === 5) return "What time of day was it when this memory happened?";
    if (s === 6) return "What was the weather like during this memory?";
    if (s === 7) return "How many people were present, including yourself?";
    if (s === 8) {
    ...
    if (s === 9) return "Now, tell us what was happening in the scene and how everyone was interacting.";
    if (s === 10) return "How were you feeling and acting in the scene?";
    if (s === 11) return "Would you prefer to see the scene from first-person or third-person?";
    if (s === 12) return "Were there any particular details that really stood out to you, and why?";
    if (s === 13) return "Were there any background sounds, noises, or animals?";
    if (s === 14) return "What genre of music best reflects the atmosphere of this scene?";
    return "";
  };
