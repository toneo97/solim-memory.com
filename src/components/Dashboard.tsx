...
interface DashboardProps {
  ...
  activeTab: 'memories' | 'drafts' | 'account';
  activeSection?: string; // for account sub-sections like 'billing', 'security'
  onNavigate: (tab: 'memories' | 'drafts' | 'account', section?: string) => void;
...
const Dashboard: React.FC<DashboardProps> = ({ activeTab, activeSection, onNavigate, onLogout, onNewMemory, onHome }) => {
  // Internal state for things that don't need URL routing (like Billing sub-tabs 'invoices' vs 'methods')
  ...
  // Derive account sub-tab from activeSection or default to 'profile'
  const accountSubTab = activeSection || 'profile';
            ...
            <div className="flex flex-col md:flex-row gap-8">
               ...
               <div className="w-full md:w-64 flex-shrink-0 space-y-2">
                 {[
                   ...
                   <button
                    key={tab.id}
                    onClick={() => onNavigate('account', tab.id)}
                    ...
                    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2">
                            ...
                            <div className="bg-gradient-to-br from-slate-900 to-slate-800 rounded-2xl p-8 text-white relative overflow-hidden shadow-xl">
                                ...
                                <div className="relative z-10">
                                    ...
                                    <div className="flex items-center gap-4">
                                         ...
                                         <button onClick={() => onNavigate('account', 'billing')} className="text-sm hover:text-white text-white/70 hover:underline transition-colors">Update method</button>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="grid md:grid-cols-2 gap-6">
                            <div className="border border-slate-100 rounded-xl p-6 hover:border-slate-300 transition-colors">
                                ...
                                <button onClick={() => onNavigate('account', 'billing')} className="text-blue-600 text-sm font-medium hover:underline">View History</button>
                            </div>
                            <div className="border border-slate-100 rounded-xl p-6 hover:border-slate-300 transition-colors">
